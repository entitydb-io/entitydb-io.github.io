# IFilterBuilderExtensions.Xor&lt;TFilter&gt; method

Returns a *TFilter* that only includes objects which only match one filter.
...
A *TFilter* that only includes objects which only match *filterA* or only match *filterB*.
